...
class multiView:
    def __init__(self, vid_num=[0, 2, 4, 6], can_chan='can0', cap_res=[1280, 720], disp_res=(3840, 2160),
        ...
        self.selforM1 = 0
        self.selforM2 = [0, len(self.vid)-1]
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.guiMode == 1:
                if self.selforM1 == 0:
                    ...
                    if self.selforM1 > len(self.vid):
                        self.selforM1 = len(self.vid)
                ...
                if refPt[0] < self.fullsize[0]/2:
                    ...
                    if self.selforM2[0] >= len(self.vid):
                        self.selforM2[0] = 0
                    ...
                    if self.selforM2[1] >= len(self.vid):
                        self.selforM2[1] = 0
    ...
    def videoCap(self):
        if self.guiMode == 0:
            ...
            if len(self.vid) < 4:
                for i in range(len(self.vid), 4):
                    self.vid_frame[i] = np.zeros((self.cap_res[1], self.cap_res[0], 3), dtype=np.uint8)
